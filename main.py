...
class InstrScr(Screen):
    def __init__(self, **kwargs):
        ...
        lbl1 = Label(text='[color=#0000FF][size=24][i]Введите имя [s]своей собаки[/s]:[/i][/size][/color]', markup=True, halign='right')
        self.in_name = TextInput(multiline=False)
        lbl2 = Label(text='[color=#003399][size=24][i]Введите возраст [s]Байдена[/s]:[/i][/size][/color]', markup=True, halign='right')
        self.in_age = TextInput(text='7', multiline=False)

        self.btn = Button(text='[size=16]Начать [s](нет, закончить)[/s][/size]', markup=True, size_hint=(0.3, 0.2), pos_hint={'center_x': 0.5})
...
class PulseScr(Screen):
    def __init__(self, **kwargs):
        ...
        lbl1 = ScrollLabel('[size=24]Считайте пульс[/size]', textcolor = '#FFFFFF')
...
class CheckSits(Screen):
    def __init__(self, **kwargs):
        ...
        lbl1 = ScrollLabel('Сделайте 30[s]000[/s] приседаний', textcolor = '#FFFFFF')
...
class PulseScr2(Screen):
    def __init__(self, **kwargs):
        ...
        self.lbl1 = ScrollLabel('Считайте пульс [s](если остались в живых)[/s]', textcolor = '#FFFFFF')
        self.lbl_sec = Seconds(15, textcolor = '#FFFFFF')
        self.lbl_sec.bind(done=self.sec_finished)
    ...
    def sec_finished(self, instance, value):
        if value:
            if self.stage == 0:
                ...
                self.stage = 1
                self.lbl1.set_text('Отдыхайте [s]а лучше работайте[/s]')
                self.lbl_sec.restart(30)
                self.in_result1.set_disabled(False)
            elif self.stage == 1:
                # закончили отдых, считаем
                self.stage = 2
                self.lbl1.set_text('Считайте пульс [s]не отрываясь от работы[/s]')
